...
public class AppManageActivity extends BaseActivity implements OnItemClickListener, OnItemSelectedListener{
	...
	protected void onResume() {
		super.onResume();
		if (isChinese()){
			iv_local_app.setImageDrawable(context.getResources().getDrawable(R.drawable.local_app));
		}else{
			iv_local_app.setImageDrawable(context.getResources().getDrawable(R.drawable.local_app_en));
		}
/*		MobclickAgent.onPageStart(TAG);
	...
	private void initData() {
		...
		if ((panel_height == 1080) && (panel_width == 1920)){
			localLayoutParams = new FrameLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.ktc_0190), getResources().getDimensionPixelSize(R.dimen.ktc_0180));
		    localLayoutParams.leftMargin = getResources().getDimensionPixelSize(R.dimen.ktc_0086);
		    localLayoutParams.topMargin = getResources().getDimensionPixelSize(R.dimen.ktc_0150);
		    this.whiteBorder.setLayoutParams(localLayoutParams);
		    this.whiteBorder.setVisibility(View.VISIBLE);
		}else{
			localLayoutParams = new FrameLayout.LayoutParams(getResources().getDimensionPixelSize(R.dimen.ktc_0190), getResources().getDimensionPixelSize(R.dimen.ktc_0200));
		    localLayoutParams.leftMargin = getResources().getDimensionPixelSize(R.dimen.ktc_0086);
		    localLayoutParams.topMargin = getResources().getDimensionPixelSize(R.dimen.ktc_0164);
		    this.whiteBorder.setLayoutParams(localLayoutParams);
		    this.whiteBorder.setVisibility(View.VISIBLE);
		}
	...
	protected void findViewById() {
		...
		iv_local_app = (ImageView)findViewById(R.id.iv_local_app);
	...
	//查询常用app的数据
	private int getNumberAi(){
		//先查询数据库，获取所有常用的app
		List<String> list = dbtools.queryAll();
		//再获取系统所有已安装的app
		Intent intent = new Intent(Intent.ACTION_MAIN);
		intent.addCategory(Intent.CATEGORY_LAUNCHER);
		List<ResolveInfo> applist = getPackageManager().queryIntentActivities(intent, 0);;
		ArrayList<AppInfo> loveApp = new ArrayList<AppInfo>();
...
*/		
	...
	public boolean filterApk(String packagenName) {
		...
		{
		    return true;
		}else if(packagenName.equals("com.android.mslauncher"))
		{
		    return true;
		...
		else if(packagenName.equals("com.android.dummyactivity"))
		{
			...
			return true;
		}else if(packagenName.equals("com.ktc.tvlauncher")){
			return true;
		}
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				case 0:
					if(isVis == REMOVE_FROM_FAV){
						...
						if (getNumberAi() >= 8){
							Toast.makeText(AppManageActivity.this, "您的常用App已满,请删除后再添加!", Toast.LENGTH_SHORT).show();
						}else{
							//添加到常用
	...
	private void showMenu(int type) {
		...
		menupopupWindow.update(0, 0, getResources().getDimensionPixelSize(R.dimen.ktc_0160),mHeight);
	}
	...
	class AppAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			//更改本地应用中的UI-LocalMM为多媒体中心，Tvsetting为电视
			if(aif.getAppname().endsWith("LocalMM")){
				viewHolder.myapp_gridview_item_tv.setText(R.string.media_center);
				viewHolder.myapp_gridview_item_flag_tv.setText(aif.getApppack());
			}else if (aif.getAppname().equals("Tvsetting")){			
				viewHolder.myapp_gridview_item_tv.setText(R.string.tv_only);
				viewHolder.myapp_gridview_item_flag_tv.setText(aif.getApppack());
			}else{	
...
/*			viewHolder.myapp_gridview_item_tv.setText(aif.getAppname());
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		if ((panel_height == 1080) && (panel_width == 1920)){
			float f1 = view.getX() + getResources().getDimensionPixelSize(R.dimen.ktc_0086);
	        float f2 = view.getY() + getResources().getDimensionPixelSize(R.dimen.ktc_0148); 
        	mainActivity.flyWhiteBorder(getResources().getDimensionPixelSize(R.dimen.ktc_0192), getResources().getDimensionPixelSize(R.dimen.ktc_0180), f1, f2);
        }else{
        	float f1 = view.getX() + getResources().getDimensionPixelSize(R.dimen.ktc_0086);
	        float f2 = view.getY() + getResources().getDimensionPixelSize(R.dimen.ktc_0162); 
        	mainActivity.flyWhiteBorder(getResources().getDimensionPixelSize(R.dimen.ktc_0192), getResources().getDimensionPixelSize(R.dimen.ktc_0202), f1, f2);
        }
